        ...
        if (err) {
          ...
          if (result.errcode == 0 && result.data && result.data.link) {
            ...
            if (result.errcode == 0 && result.msg == 'success') {
              f = -3
              if (((result.data && result.data.msg) || '').indexOf('再来') >= 0) {
                $.log(`😂账号${ac.no}：本阶段已无任务，${result.data.msg}`)
              } else {
                $.log(`🚫账号${ac.no}：阅读已限制，上午黑的，下午4点后解封的话，跑脚本前先手动阅读2篇文章`)
              }
            } else {
              $.logErr(`🚫账号${ac.no}：获取key回执失败，响应结果如下：\n${JSON.stringify(result, null, 2)}`)
              f = -2
              await $.wait(1500)
            }
          }
